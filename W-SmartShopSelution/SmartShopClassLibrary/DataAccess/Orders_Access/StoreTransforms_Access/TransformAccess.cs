...
namespace Library
{
    ...
    {
        /// <summary>
        /// Add Transform to the database 
        /// </summary>
        /// <param name="transform"></param>
        /// <param name="db"></param>
        /// <returns>Transform with the new database Id</returns>
        public static TransformModel AddTransformToTheDatabase(TransformModel transform,string db)
        {
            using (IDbConnection connection = new System.Data.SqlClient.SqlConnection(GlobalConfig.CnnVal(db)))
            {
                var p = new DynamicParameters();
                p.Add("@StaffId", transform.Staff.Id); 
                p.Add("@StoreId", transform.Store.Id);
                p.Add("@Date", transform.Date);
                p.Add("@TotalMoney", transform.TotalMoney);
                p.Add("@ToStoreId", transform.ToStore.Id);
                p.Add("@Id", 0, dbType: DbType.Int32, direction: ParameterDirection.Output);
                connection.Execute("dbo.spTransform_Create", p, commandType: CommandType.StoredProcedure);
                transform.Id = p.Get<int>("@Id");
            }
            return transform;
        }
